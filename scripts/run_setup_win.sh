...
# Check if local_env exists
if [ -f "$LOCAL_ENV_PATH" ]; then
  echo "Deleting local_env: $LOCAL_ENV_PATH"
  rm "$LOCAL_ENV_PATH"
else
...
KEY=$(python -c 'import random; import string; print("".join(random.SystemRandom().choice(string.ascii_letters + string.digits + "!@#$%^&*(-_=+)") for _ in range(50)))')

#.local file update
echo "SECRET_KEY='$KEY'" >> .\/.envs\/.local
